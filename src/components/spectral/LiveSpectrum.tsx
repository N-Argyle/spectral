...
export default function LiveSpectrum({
  ...
  useEffect(() => {
    ...
    const calibKey = blackoutCalibrationData
      ? `calib-${blackoutCalibrationData.width}x${blackoutCalibrationData.height}`
      : "no-calib";
    ...
    const frameMax = Math.max(
      ...processedData.filter((v) => typeof v === "number" && !isNaN(v)),
      0.01,
    );
    ...
    console.log(
      "LiveSpectrum: Updating with processed data, length:",
      safeProcessedData.length,
    );
  ...
  useEffect(() => {
    ...
    console.log(
      "LiveSpectrum: blackoutCalibrationData changed:",
      calibKey ?? "null",
    );
  ...
  const handleCanvasReady = useCallback(
    (
      ...
      try {
        ...
        ctx.fillRect(
          0,
          displayHeight - 20 - referenceHeight,
          displayWidth,
          referenceHeight,
        );
